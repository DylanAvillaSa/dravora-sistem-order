...
export default function ProductScroll2({
  filteredProducts,
  cart,
  addToCart,
  onClickProduct,
  ...
  const handleScroll = () => {
    ...
    const index = Math.round(el.scrollLeft / (el.offsetWidth * 0.6));
        ...
        {filteredProducts.map((item, index) => {
          ...
          return (
            <motion.div
              key={item.id}
              whileTap={{ scale: 0.97 }}
              onClick={() => onClickProduct(item)} // 🔥 OPEN MODAL
            ...
            >
              <div className="w-full text-center relative">
                ...
                {item.img ? (
                  <img
                    src={item.img}
                    className="h-40 w-full rounded-2xl object-cover mb-2"
                  />
                ) : (
                  <div className="h-40 w-full rounded-2xl mb-2 bg-gray-300 flex items-center justify-center">
